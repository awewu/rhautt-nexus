...
async function run() {
  ...
  try {
    ...
    if (!customerId || lead.duplicate)
      throw new Error('Quote smoke could not create its customer graph');
    ...
    const quote = await request(
      ...
      '/api/v2/quotation',
      {
        ...
        body: JSON.stringify({
          ...
          items: [
            { sku: 'SMOKE-1', name: 'Smoke item', unitPrice: 10000, unitCost: 7000, quantity: 1 },
          ],
    ...
    if (!quotationId || !quote.projectId)
      throw new Error('Quote persist returned no quotation/project id');

    const listed = await request(
      PROXY,
      `/api/v2/quotation?opportunityId=${opportunityId}`,
      {},
      login.token
    );
    if (!listed.some((item) => item.id === quotationId))
      throw new Error('Proxy quote list did not return the persisted quote');

    const locked = await request(
      PROXY,
      `/api/v2/quotation/${quotationId}/lock`,
      {
        method: 'POST',
        body: JSON.stringify({}),
      },
    ...
    if (locked.status !== 'locked' || !locked.quotationLock?.locked)
      throw new Error('Proxy quote lock did not freeze the quote');
    ...
    if (row.audit_count !== 2 || row.outbox_count !== 2)
      throw new Error(`quote database proof failed: ${JSON.stringify(row)}`);
    ...
    if (customerId) {
      ...
      await db.query('SELECT set_config($1, $2, true)', ['app.tenant_id', tenantId]);
      if (quotationId)
        await db.query('DELETE FROM rhautt_nexus.quotations WHERE id = $1', [quotationId]);
      ...
      await db.query('DELETE FROM rhautt_nexus.audit_logs WHERE resource_id = ANY($1::text[])', [
        [customerId, opportunityId, quotationId].filter(Boolean),
      ]);
      await db.query(
