...
async function run() {
  ...
  for (const p of PROJECTS) {
    ...
    const { rows: ex } = await client.query(
      'SELECT id FROM rhautt_nexus.bim_projects WHERE tenant_id=$1 AND customer_name=$2',
      [tenantId, p.name]
    );
    ...
    await client.query(
      ...
      [
        ...
        p.name,
        p.city,
        JSON.stringify({ area: p.area }),
        JSON.stringify({ total: p.total }),
        p.paid,
        p.fams.join(','),
        JSON.stringify(CHECK(MS, p.done)),
        p.installer,
      ]
    );
    created++;
  ...
  const {
    ...
    'SELECT count(*)::int count FROM rhautt_nexus.bim_projects WHERE tenant_id=$1',
    [tenantId]
  );
