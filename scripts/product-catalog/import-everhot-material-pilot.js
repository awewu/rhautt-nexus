...
const SOURCE_PATH = path.join(ROOT, 'docs', 'dev', 'everhot-product-library-field-extraction-0725.xlsx');
...
const TECHNICAL_FIELDS = Object.freeze([
  '毛重', '毛重单位', '长', '宽', '高', '净重', '电击防护类型', '防水等级', '额定功率',
  '额定电压/频率', '加热管输入功率', '燃气类别', '低压侧/高压侧最大允许压力',
  '吸气侧允许工作压力', '水箱工作允许过压', '最高出水温度', '制冷剂/充注量',
  '工作条件', '执行标准',
]);
...
function readPilotRows(sourcePath = SOURCE_PATH) {
  ...
  return SOURCE_ROWS.map((sourceRow) => {
    ...
    if (clean(row.brandCode).toLowerCase() !== 'everhot') throw new Error(`源 Excel 第 ${sourceRow} 行不是恒热产品`);
    return row;
  });
}
...
function readinessFor(row) {
  ...
  return Object.fromEntries(Object.entries(result).map(([key, value]) => [key, Object.fromEntries(Object.entries(value).filter(([, item]) => item !== undefined))]));
}
...
function makeToken() {
  ...
  return jwt.sign({ userId: ACTOR_ID, tenantId: TENANT_ID, role: 'platform_admin' }, process.env.JWT_SECRET, { expiresIn: '20m' });
}
...
async function apiRequest(pathname, token, options = {}) {
  ...
  let payload;
  try { payload = body ? JSON.parse(body) : null; } catch { payload = { raw: body.slice(0, 500) }; }
...
async function ensureCategories(token, rows, apply) {
  const payload = await apiRequest('/brand-product-categories?brandCode=everhot&metrics=false', token);
  ...
  for (const row of rows) {
    ...
    required.set(`${rootCode}:${category.code}`, { rootCode, sourceName: clean(row['主分类候选']), ...category });
  }
  ...
  for (const [key, item] of required) {
    ...
    let category = existing.find((candidate) => candidate.parentId === root.id && candidate.code === item.code);
    if (!category && !apply) {
      category = { id: `dry-run:${key}`, parentId: root.id, code: item.code, nameCn: item.sourceName, showOnWebsite: false };
    }
    if (!category && apply) {
      const created = await apiRequest('/brand-product-categories', token, {
        method: 'POST',
        body: JSON.stringify({
          brandCode: 'everhot', parentId: root.id, code: item.code, slug: item.code,
          nameCn: item.sourceName, nameEn: item.nameEn, sortOrder: 100, status: 'active',
          showOnWebsite: false, description: '恒热物料试导入所需分类；试验阶段不进入官网公开目录。',
        }),
      });
...
async function main() {
  ...
  const productsPayload = await apiRequest(`/product-catalog/devices?tenantId=${TENANT_ID}&brand=everhot&page=1&pageSize=500`, token);
  const existingBySku = new Map(responseItems(productsPayload).map((item) => [item.sku, item]));
  rows.forEach((row) => assertSafeExisting(existingBySku.get(clean(row.skuCode)), clean(row.skuCode)));
  ...
  console.table(plan.map((item) => ({ sku: item.sku, model: item.spec.officialModel, category: item.category, status: item.status, published: item.published })));
  if (!APPLY) {
    console.log(`Dry-run: ${plan.length} products. Pass --apply to import through the protected NestJS API.`);
    return;
  }
  ...
  for (const dto of plan) {
    ...
    await apiRequest('/product-catalog/devices', token, { method: 'POST', body: JSON.stringify(dto) });
    if (existed) updated += 1;
    else created += 1;
  }

  const verifiedPayload = await apiRequest(`/product-catalog/devices?tenantId=${TENANT_ID}&brand=everhot&status=inactive&page=1&pageSize=500`, token);
  const verified = responseItems(verifiedPayload).filter((item) => item?.meta?.productLibrary?.batchCode === BATCH_CODE);
  const unsafe = verified.filter((item) => item.status !== 'inactive' || item.published !== false || item?.meta?.productLibrary?.autoPublish !== false);
  if (verified.length !== SOURCE_ROWS.length || unsafe.length) {
    throw new Error(`导入后校验失败：找到 ${verified.length}/${SOURCE_ROWS.length} 条，非安全状态 ${unsafe.length} 条`);
  }
  console.log(JSON.stringify({ batchCode: BATCH_CODE, created, updated, verified: verified.length, published: 0 }, null, 2));
}
