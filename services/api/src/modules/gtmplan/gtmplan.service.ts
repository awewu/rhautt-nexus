...
export class GtmplanService {
  ...
  async createCampaign(
    actor: JwtPayload,
    dto: { name?: string; buType?: string; buRef?: string; period?: string; budget?: number }
  ) {
      ...
      async (em) => {
        ...
        const row = await repo.save(
          repo.create({
            ...
            name: dto.name!,
            buType: dto.buType ?? null,
            buRef: dto.buRef ?? null,
        ...
        await writeAudit(em, {
          ...
          action: 'gtm.campaign.create',
          resourceType: 'gtm_campaign',
          resourceId: row.id,
          afterState: { name: dto.name, period: dto.period, budget: Number(dto.budget) || 0 },
        });
  ...
  async updateCampaign(
    actor: JwtPayload,
    id: string,
    patch: { spend?: number; attributedRevenue?: number; status?: string }
  ) {
    return withRlsTransaction(
      this.ds,
      async (em) => {
        ...
        if (patch.attributedRevenue != null)
          upd.attributedRevenue = Number(patch.attributedRevenue);
        if (patch.status) upd.status = patch.status;
        await em.getRepository(GtmCampaignEntity).update({ id, tenantId: actor.tenantId }, upd);
        await writeAudit(em, {
          tenantId: actor.tenantId,
          actorUserId: actor.userId,
          action: 'gtm.campaign.update',
          resourceType: 'gtm_campaign',
          resourceId: id,
          afterState: {
            spend: patch.spend,
            attributedRevenue: patch.attributedRevenue,
            status: patch.status,
          },
        });
        const row = await em
          .getRepository(GtmCampaignEntity)
          .findOne({ where: { id, tenantId: actor.tenantId } });
  ...
  async mroiSummary(actor: JwtPayload) {
    return withRlsTransaction(
      this.ds,
      async (em) => {
        const rows: Array<{ budget: string; spend: string; revenue: string; n: string }> = await em
          .query(
            `SELECT COALESCE(SUM(budget),0) budget, COALESCE(SUM(spend),0) spend, COALESCE(SUM(attributed_revenue),0) revenue, COUNT(*) n
           FROM rhautt_nexus.gtm_campaign WHERE tenant_id = $1`,
            [actor.tenantId]
          )
          .catch(() => []);
        ...
        return {
          campaigns: Number(r.n) || 0,
          budget: Number(r.budget) || 0,
          spend,
          attributedRevenue: revenue,
          mroi: spend > 0 ? revenue / spend : null,
        };
      },
      this.scope(actor)
    );
  ...
  async upsertOkr(
    ...
    if (!dto.objective || !['group', 'business_unit', 'function'].includes(String(dto.level)))
      throw new BadRequestException('objective and valid level required');
    return withRlsTransaction(
      this.ds,
      async (em) => {
        ...
        if (dto.id) {
          ...
          await writeAudit(em, {
            tenantId: actor.tenantId,
            actorUserId: actor.userId,
            action: 'gtm.okr.update',
            resourceType: 'gtm_okr',
            resourceId: dto.id,
            afterState: {
              level: dto.level,
              objective: dto.objective,
  ...
  async listOkrs(actor: JwtPayload, level?: string) {
    return withRlsTransaction(
      ...
      async (em) => {
        ...
        return {
          okrs: await em
            .getRepository(GtmOkrEntity)
            .find({ where, order: { level: 'ASC', updatedAt: 'DESC' } }),
        };
      },
      this.scope(actor)
    );
  ...
  async okrSummary(actor: JwtPayload) {
    return withRlsTransaction(
      this.ds,
      async (em) => {
        const rows: Array<{ level: string; avg: string; n: string }> = await em
          .query(
            `SELECT level, AVG(progress) avg, COUNT(*) n FROM rhautt_nexus.gtm_okr WHERE tenant_id = $1 GROUP BY level`,
            [actor.tenantId]
          )
          .catch(() => []);
        return {
