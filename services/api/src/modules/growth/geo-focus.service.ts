...
export class GeoFocusService {
      ...
      async (em) => {
        ...
        if (dto.id) {
          await repo.update({ id: dto.id, tenantId: actor.tenantId }, {
            ...
            assetGaps,
            probeStrategy: (dto.probeStrategy ?? {}) as any,
            variantStrategy: (dto.variantStrategy ?? {}) as any,
            updatedAt: new Date(),
          } as any);
    ...
    return withRlsTransaction(
      ...
      async (em) => {
        ...
        return {
          targets: await em
            .getRepository(GeoTargetEntity)
            .find({ where, order: { priorityScore: 'DESC' }, take: 200 }),
        };
      },
      this.scope(actor)
    );
  ...
  async listProbePool(
    actor: JwtPayload,
    query: { category?: string; probeType?: string; segment?: string; engine?: string } = {}
  ) {
    return withRlsTransaction(
      this.ds,
      async (em) => {
        ...
        if (query.probeType)
          qb.andWhere('t.probe_type = :probeType', {
            probeType: normalizeProbeType(query.probeType),
          });
    ...
    return withRlsTransaction(
      ...
      async (em) => {
        ...
        const all = await repo.find({
          where: { tenantId: actor.tenantId, category: dto.category } as any,
        ...
        return {
          ...
          skipped,
          targets: all,
          summary: summarizeProbePool(all),
        };
      },
      this.scope(actor)
    );
      ...
      async (em) => {
        ...
        if (existing) {
          await repo.update(
            { id: existing.id },
            {
              reach: existing.reach + (Number(dto.reach) || 0),
              cited: existing.cited + (Number(dto.cited) || 0),
              recommended: existing.recommended + (Number(dto.recommended) || 0),
              lead: existing.lead + (Number(dto.lead) || 0),
              updatedAt: new Date(),
            }
          );
        ...
        const row = await repo.save(
          repo.create({
            ...where,
            reach: Number(dto.reach) || 0,
            cited: Number(dto.cited) || 0,
            recommended: Number(dto.recommended) || 0,
            lead: Number(dto.lead) || 0,
          } as any)
        );
        return { asset: row };
      },
  ...
  async cognitionFunnel(actor: JwtPayload, category?: string) {
    return withRlsTransaction(
      ...
      async (em) => {
        ...
        const rows: Array<{ reach: string; cited: string; recommended: string; lead: string }> =
          await em.query(sql, params).catch(() => []);
  ...
  async reallocate(actor: JwtPayload, adjustments: Array<{ id: string; deltaPriority: number }>) {
    if (!Array.isArray(adjustments) || !adjustments.length)
      throw new BadRequestException('adjustments required');
    return withRlsTransaction(
      this.ds,
      async (em) => {
        const repo = em.getRepository(GeoTargetEntity);
        for (const a of adjustments) {
          ...
          if (t)
            await repo.update(
              { id: t.id },
              {
                priorityScore: Number(t.priorityScore) + Number(a.deltaPriority || 0),
                status: 'active',
                updatedAt: new Date(),
              }
            );
...
export function summarizeProbePool(targets: GeoTargetEntity[]) {
  ...
  const byStage = Object.fromEntries(INTENT_STAGES.map((stage) => [stage, 0])) as Record<
    string,
    number
  >;
  let highPriority = 0;
...
export function buildProbeSeeds(
  categoryLabel: string,
  dto: { engine?: string; region?: string; segment?: string }
) {
  ...
  return [
    ...
    seed(
      'comparison',
      `Rheem 和国产${categoryLabel}品牌有什么区别`,
      'compare',
      'compare',
      segment,
      90
...
function seed(
  ...
  return {
    ...
    probeStrategy: {
      cadence: 'weekly',
      expansion: ['原始问题', '语义改写', '角色视角', '多引擎复测'],
    },
  };
}
