...
test('GEO loop exposes one honest next action for every phase', () => {
  assert.equal(buildGeoLoopState({ status: 'baseline', baselineCitedRate: null }).nextAction, 'wait-for-baseline');
  assert.equal(buildGeoLoopState({ status: 'baseline', baselineCitedRate: 0 }).nextAction, 'generate-fact-grounded-draft');
  assert.equal(
    buildGeoLoopState({ status: 'baseline', baselineCitedRate: 0, copyAssetId: 'copy-1' }, { status: 'draft' }).nextAction,
    'approve-draft',
  );
  assert.equal(
    buildGeoLoopState({ status: 'baseline', baselineCitedRate: 0, copyAssetId: 'copy-1' }, { status: 'approved' }).nextAction,
    'record-publication-evidence',
  );
  assert.equal(
    buildGeoLoopState({ status: 'content-linked', baselineCitedRate: 0, copyAssetId: 'copy-1', publicationUrl: 'https://example.com/a' }, { status: 'published' }).nextAction,
    'verify-lift',
  );
  ...
  assert.deepEqual(buildGeoLoopState({ status: 'improved', lift: 10 }), {
    phase: 'completed', nextAction: 'review-result', terminal: true,
  });
});
...
test('publication evidence accepts only absolute HTTP(S) URLs', () => {
  ...
  assert.equal(normalizePublicationUrl('https://www.rheem.com.cn/article'), 'https://www.rheem.com.cn/article');
});
