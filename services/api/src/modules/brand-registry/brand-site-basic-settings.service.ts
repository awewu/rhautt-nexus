...
export class BrandSiteBasicSettingsService {
    ...
    return withRlsTransaction(
      ...
      async (em) => {
        ...
        await this.audit(
          em,
          user,
          'brand-site-basic-settings.update',
          saved.id,
          before,
          this.snapshot(saved)
        );
  ...
  publicGet(siteCodeInput: string) {
    ...
    if (!tenantId || !UUID_RE.test(tenantId))
      throw new NotFoundException('website public tenant is not configured');
    return withRlsTransaction(
      this.ds,
      async (em) => {
        const site = await this.findSite(em, tenantId, siteCode);
        const row = await em
          .getRepository(BrandSiteBasicSettingsEntity)
          .findOneBy({ tenantId, siteId: site.id });
  ...
  private normalizedPatch(
    input: BrandSiteBasicSettingsInput
  ): Partial<BrandSiteBasicSettingsEntity> {
  ...
  private normalizeSection(sectionInput: string): SettingsSection {
    ...
    if (!SECTION_SET.has(section))
      throw new BadRequestException('basic settings section is invalid');
    return section;
  }
  ...
  private validateLink(key: string, value: string) {
    ...
    if (!['http:', 'https:'].includes(url.protocol)) {
      throw new BadRequestException(
        `${key} only allows http/https, relative, mailto, or tel links`
      );
    }
  }
