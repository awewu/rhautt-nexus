...
export class CdpService implements OnModuleInit {
  ...
  // 不落原始 PII、不建端用户管理界面。
  onModuleInit(): void {
    this.eventBus.subscribe('diagnosis.completed', (e: OutboxEventEntity) =>
      this.ingestFromDiagnosis(e)
    );
  ...
  private async ingestFromDiagnosis(event: OutboxEventEntity) {
    ...
    const attrs = {
      ...
      systems: p.systems ?? [],
      sourceSurface: p.sourceSurface ?? null,
    ...
    await withRlsTransaction(
      ...
      { tenantId, actorId: 'system:cdp-ingest' }
    ).catch((err) => this.logger.warn(`ingestFromDiagnosis 失败: ${err?.message || err}`));
  }
  ...
  private async markConverted(event: OutboxEventEntity) {
    ...
    await withRlsTransaction(
        ...
        if (existing) {
          await repo.update({ id: existing.id }, {
            attributes: {
              ...(existing.attributes || {}),
              stage: 'signed',
              dealSignedAt: new Date().toISOString(),
            },
            updatedAt: new Date(),
          } as any);
        }
      ...
      { tenantId, actorId: 'system:cdp-ingest' }
    ).catch((err) => this.logger.warn(`markConverted 失败: ${err?.message || err}`));
  }
  ...
  async upsertProfile(
    ...
    return withRlsTransaction(
      ...
      async (em) => {
        ...
        const profile = phoneHash
          ? await repo.findOne({ where: { tenantId: actor.tenantId, phoneHash } })
          : null;
        const patch: any = {
        ...
        if (profile) {
          ...
          await writeAudit(em, {
            ...
            action: 'cdp.profile.update',
            resourceType: 'cdp_end_user_profile',
            resourceId: profile.id,
            afterState: { source: patch.source, hasPhone: !!phoneHash },
          });
          return { id: profile.id, updated: true };
        }
        const saved = await repo.save(
          repo.create(patch as Partial<CdpProfileEntity>) as CdpProfileEntity
        );
        await writeAudit(em, {
          tenantId: actor.tenantId,
          actorUserId: actor.userId,
          action: 'cdp.profile.create',
          resourceType: 'cdp_end_user_profile',
          resourceId: saved.id,
          afterState: { source: patch.source, hasPhone: !!phoneHash },
        });
  ...
  async createSegment(
    actor: JwtPayload,
    dto: { code?: string; name?: string; rule?: Record<string, unknown> }
  ) {
  ...
  async listSegments(actor: JwtPayload) {
    return withRlsTransaction(
      this.ds,
      async (em) => {
        const segments = await em
          .getRepository(CdpSegmentEntity)
          .find({ where: { tenantId: actor.tenantId }, order: { code: 'ASC' } });
        return { segments };
      },
      { tenantId: actor.tenantId, actorId: actor.userId, role: actor.role }
    );
  ...
  async recordConsent(
    ...
    if (!dto.profileId || !dto.purpose)
      throw new BadRequestException('profileId and purpose are required');
    return withRlsTransaction(
      this.ds,
      async (em) => {
        await em.getRepository(CdpConsentEntity).save(
          em.getRepository(CdpConsentEntity).create({
            tenantId: actor.tenantId,
        ...
        await em
          .getRepository(CdpProfileEntity)
          .update(
