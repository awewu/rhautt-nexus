...
function fixture(rows: any[] = []) {
  ...
  productRepo.seed(
    {
      id: 'pl1',
      tenantId: 'product-library',
      name: 'Rheem library product',
      sku: 'SKU-L1',
      brandCode: 'rheem',
      status: 'active',
      recordStatus: 'active',
      dataReadinessStatus: 'fact_verified',
      published: true,
      deletedAt: null,
    },
    {
  ...
  contentAssetRepo.seed(
    {
      ...
      status: 'active',
      archivedAt: null,
      title: 'Verified lifestyle image',
      assetType: '封面图',
      brandSlug: 'rheem',
      channel: 'wechat',
      summary: 'usable content asset',
      tags: ['节能'],
      fileArtifactId: 'f1',
      fileUrl: '',
      thumbnailUrl: '',
    ...
    },
    {
      ...
      status: 'active',
      archivedAt: null,
      title: 'Reusable generic image',
      assetType: '正文配图',
      brandSlug: null,
      channel: 'seo',
      summary: 'generic content asset',
      tags: ['通用'],
      fileArtifactId: 'f3',
      fileUrl: '',
      thumbnailUrl: '',
    ...
    },
    {
      ...
      tenantId: 't1',
      status: 'active',
      archivedAt: new Date('2026-08-12'),
  ...
  const { svc } = fixture([
    {
      ...
      tenantId: 't1',
      status: 'in_review',
      factRefs: [{ type: 'fact', id: 'f1', verified: true }],
    },
  ]);
  await assert.rejects(
    () => svc.createPublishTask(actor, 'c1', { channel: 'official_site' }),
    /审核通过/
  );
});
...
test('createPublishTask blocks approved content without fact refs', async () => {
  ...
  await assert.rejects(
    () => svc.createPublishTask(actor, 'c2', { channel: 'official_site' }),
    /无事实源/
  );
});
...
test('createPublishTask blocks approved content with unverified fact refs', async () => {
  const { svc } = fixture([
    { id: 'c3', tenantId: 't1', status: 'approved', factRefs: [{ type: 'fact', id: 'f1' }] },
  ]);
  await assert.rejects(
    () => svc.createPublishTask(actor, 'c3', { channel: 'official_site' }),
    /事实源未通过校验/
  );
});

test('createPublishTask creates a channel task and keeps content approved', async () => {
  const { svc, repo, taskRepo } = fixture([
    {
      id: 'c4',
      tenantId: 't1',
      status: 'approved',
  ...
  const result: any = await svc.createPublishTask(actor, 'c4', {
...
test('completePublishTask requires evidence and marks content as published', async () => {
  ...
  const created: any = await svc.createPublishTask(actor, 'c4b', {
    channel: 'wechat',
    targetName: '官方公众号',
  });
  await assert.rejects(
    () => svc.completePublishTask(actor, created.task.id, {}),
    /发布链接|发布凭证/
  );
  const result: any = await svc.completePublishTask(actor, created.task.id, {
    evidenceUrl: 'https://example.test/post/1',
  });
  assert.equal(result.status, 'published');
  assert.equal(
    (taskRepo.rows.find((x: any) => x.id === created.task.id) as any).status,
    'published'
  );
...
test('rejection requires a review note and stores feedback history', async () => {
  ...
  await assert.rejects(
    () => svc.decide(actor, 'c4c', 'rejected', { rejectionReason: 'fact_missing' }),
    /修改意见/
  );
  const result: any = await svc.decide(actor, 'c4c', 'rejected', {
    rejectionReason: 'fact_missing',
    reviewNote: '缺少产品参数来源，请补充手册或产品事实。',
  });
...
test('bindFactRefs validates manual fact artifacts', async () => {
  ...
  assert.equal(result.gate.passed, true);
  assert.equal(
    (repo.rows.find((x: any) => x.id === 'c8') as any).factRefs[0].label,
    'manual-fact.pdf'
  );
});
...
test('productionContext exposes only enabled products and content asset library items', async () => {
  const { svc } = fixture();
  const result: any = await svc.productionContext(actor, {
    brandCode: 'rheem',
    channel: 'wechat',
    productTenantId: 'product-library',
    limit: 10,
  });
  assert.deepEqual(
    result.products.map((item: any) => item.id),
    ['pl1']
  );
  assert.equal(
    result.products.some((item: any) => ['p0', 'p1', 'p2', 'p3', 'p4'].includes(item.id)),
    false
  );
  assert.deepEqual(
    result.materials.map((item: any) => item.id),
    ['ca1', 'ca3']
  );
  assert.equal(
    result.materials.some((item: any) => item.id === 'f1' || item.label === 'manual-fact.pdf'),
    false
  );
  assert.equal(
    result.factSources.some(
      (item: any) =>
        item.type === 'manual' && item.id === 'f1' && item.label === 'Verified lifestyle image'
    ),
    true
  );
});
...
test('productionContext can browse enabled products before a brand is selected', async () => {
  ...
  assert.equal(
    result.materials.some((item: any) => item.id === 'ca3'),
    true
  );
});
...
test('createPublishTask still blocks after binding pending fact refs', async () => {
  ...
  await assert.rejects(
    () => svc.createPublishTask(actor, 'c9', { channel: 'official_site' }),
    /事实源未通过校验/
  );
});
...
test('list returns production next actions for the content factory', async () => {
  const { svc } = fixture([
    ...
    },
    {
      ...
      title: '可发布稿',
      status: 'approved',
      factRefs: [{ type: 'manual', id: 'f1', verified: true }],
      updatedAt: new Date('2026-08-10'),
    },
  ]);
