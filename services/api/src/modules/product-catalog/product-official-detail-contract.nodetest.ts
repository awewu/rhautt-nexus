...
test('product content upsert persists and readback returns official detail html', async () => {
  ...
  const sanitized =
    '<p>Official detail</p><img src="/uploads/detail-750.jpg" alt="" loading="lazy">';
...
test('public product projection exposes official detail html from published content', () => {
  const html =
    '<p>Official detail</p><img src="/api/v2/file-artifact/11111111-1111-4111-8111-111111111111/content" />';
...
test('public product projection only uses uploaded main asset, not legacy meta image fallback', () => {
  ...
  const withoutUpload = (serviceFixture().service as any).publicProductProjection(
    legacyOnly,
    'zh-CN',
    null
  );
  ...
  assert.equal(
    projected.image,
    '/api/v2/brand/everhot/products/product-2/images/22222222-2222-4222-8222-222222222222'
  );
  assert.equal(
    projected.mainImage.url,
    '/api/v2/brand/everhot/products/product-2/images/22222222-2222-4222-8222-222222222222'
  );
});
...
test('product content upsert rewrites protected artifact image urls to public product image urls', async () => {
  ...
  const saved = await service.upsertContent('product-1', {
    ...
    status: 'published',
    officialDetailHtml:
      '<img src="/api/v2/file-artifact/33333333-3333-4333-8333-333333333333/content" alt="Detail">',
  });
...
test('product content upsert sanitizes official detail html before persistence', async () => {
  ...
  assert.match(
    html,
    /<a href="https:\/\/example\.com\/detail" rel="noopener noreferrer" target="_blank">good link<\/a>/
  );
  assert.match(html, /<img src="\/uploads\/detail-750\.jpg" alt="Detail" loading="lazy">/);
  assert.match(html, /video text/);
  assert.doesNotMatch(
    html,
    /script|iframe|onerror|onclick|onmouseover|javascript:|data:image|blob:|style=|<video/i
  );
});
