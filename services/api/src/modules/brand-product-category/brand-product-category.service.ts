...
export class BrandProductCategoryService {
  ...
  async list(brandCodeInput: unknown, parentIdInput?: unknown, metricsInput?: unknown) {
    ...
    if (wantsLazy) {
      if (parentId) {
        ...
        if (!parent)
          throw new NotFoundException('Parent category does not exist or has been deleted.');
      }
      const visible = items.filter((row) => (parentId ? row.parentId === parentId : !row.parentId));
      const projected = wantsMetrics
        ? this.withTreeTableMetrics(
            visible,
            items,
            await this.productCatalog.listRawByBrand(brandCode)
          )
        : visible;
      return {
        success: true,
  ...
  async publicList(brandCodeInput: unknown) {
    ...
    const items = rows
      .filter((row) => categoryWebsiteChainVisible(row, rows))
  ...
  update(id: string, input: BrandProductCategoryInput) {
    ...
    if (!Object.keys(patch).length && input.parentId === undefined)
      throw new BadRequestException('No category fields to update.');
    ...
    return this.ds.transaction(async (em) => {
      ...
      if (patch.code && patch.code !== row.code) {
        await this.assertCodeUnique(
          em,
          row.brandCode,
          input.parentId !== undefined ? row.parentId : row.parentId,
          patch.code,
          row.id
        );
      }
  ...
  async usage(id: string) {
    ...
    const canDelete =
      childCategoryCount === 0 && exactBoundProductCount === 0 && descendantBoundProductCount === 0;
    return {
      success: true,
      data: {
        categoryId: row.id,
  ...
  delete(id: string) {
    return this.ds.transaction(async (em) => {
      ...
      const descendantBoundProductCount = await this.countDescendantBoundProducts(
        row,
        repo,
        await this.productCatalog.listRawByBrand(row.brandCode)
      );
  ...
  private inputPatch(
    input: BrandProductCategoryInput,
    creating: boolean
  ): Partial<BrandProductCategoryEntity> {
    const patch: Partial<BrandProductCategoryEntity> = {};
    if (creating || input.code !== undefined) patch.code = normalizeCode(input.code, 'code');
    if (creating || input.nameCn !== undefined)
      patch.nameCn = normalizeRequiredText(input.nameCn, 'nameCn');
    ...
    if (creating || input.sortOrder !== undefined)
      patch.sortOrder = normalizeSortOrder(input.sortOrder);
    if (creating || input.status !== undefined) patch.status = normalizeStatus(input.status);
    if (creating || input.showOnWebsite !== undefined)
      patch.showOnWebsite = normalizeBoolean(input.showOnWebsite, true);
    if (input.description !== undefined)
      patch.description = normalizeNullableText(input.description);
    return patch;
  }
  ...
  private async resolveLevel(
    ...
    if (!parent)
      throw new BadRequestException('Parent category does not exist under the selected brand.');
  ...
  private async assertMoveAllowed(
    ...
    if (parentId === row.id)
      throw new BadRequestException('Category cannot be moved under itself.');
    if (!parentId) return;
    ...
    if (!parent)
      throw new BadRequestException('Parent category does not exist under the selected brand.');
    for (
      let cursor: BrandProductCategoryEntity | undefined = parent;
      cursor;
      cursor = cursor.parentId ? byId.get(cursor.parentId) : undefined
    ) {
      if (cursor.id === row.id)
        throw new BadRequestException('Category move would create a cycle.');
    }
  }
  ...
  private countFrontendVisibleProducts(
    ...
    return products.filter((product) => isProductCountedForFrontendCategory(product, category))
      .length;
  }
  ...
  private countExactBoundProducts(
    ...
    return products.filter((product) => isProductExactlyBoundToCategory(product, category.id))
      .length;
  }
  ...
  private withTreeTableMetrics(
    ...
    return rows.sort(compareCategories).map((row) => {
      ...
      return {
        ...
        canDelete:
          childCategoryCount === 0 && directProductCount === 0 && descendantProductCount === 0,
      };
    });
  }
}
...
function normalizeCode(value: unknown, field: string): string {
  ...
  if (!CODE_RE.test(code))
    throw new BadRequestException(`${field} must use lowercase letters, numbers, and hyphens.`);
  return code;
}
...
function normalizeStatus(value: unknown): BrandProductCategoryStatus {
  const status = (
    value === undefined ? 'active' : String(value).trim().toLowerCase()
  ) as BrandProductCategoryStatus;
...
function categoryWebsiteChainVisible(
  row: BrandProductCategoryEntity,
  rows: BrandProductCategoryEntity[]
): boolean {
  ...
  return (
    ancestry.length > 0 &&
    ancestry.every(
      (item) => item.status === 'active' && item.showOnWebsite === true && !item.deletedAt
    )
  );
}
...
function withPublicAncestry(
  rows: BrandProductCategoryEntity[]
): Array<Omit<PublicBrandProductCategoryNode, 'children'>> {
  const byId = new Map(rows.map((row) => [row.id, row]));
  return rows
    .sort(compareCategories)
    .map((row) => publicCategoryProjection(row, categoryPath(row, byId)));
}

function buildPublicTree(
  rows: Array<Omit<PublicBrandProductCategoryNode, 'children'>>
): PublicBrandProductCategoryNode[] {
...
function isProductCountedForFrontendCategory(
  ...
  if (category.level >= 1 && category.level <= 3) {
    return (
      isLegacyCategoryLevelBound(product, category.id, category.level) ||
...
function isProductExactlyBoundToCategory(
  product: Record<string, unknown>,
  categoryId: string
): boolean {
  if (product.status === 'archived') return false;
  ...
  const brand = String(product.brand || '')
    ...
    .toLowerCase();
  const brandMeta =
    brand && meta[brand] && typeof meta[brand] === 'object' && !Array.isArray(meta[brand])
  ...
  if (meta.primaryCategoryId === categoryId || brandMeta.primaryCategoryId === categoryId)
    return true;
  if (
    bindings.some(
      (binding) =>
        ...
        (binding as Record<string, unknown>).categoryId === categoryId
    )
  )
    return true;
  return false;
...
function legacyProductCategorySignals(product: Record<string, unknown>): string[] {
  const meta =
    product.meta && typeof product.meta === 'object' && !Array.isArray(product.meta)
      ...
      : {};
  const spec =
    product.spec && typeof product.spec === 'object' && !Array.isArray(product.spec)
...
function isLegacyCategoryLevelBound(
  product: Record<string, unknown>,
  categoryId: string,
  level: number
): boolean {
  ...
  const brandMeta =
    brand && meta[brand] && typeof meta[brand] === 'object' && !Array.isArray(meta[brand])
      ...
      : {};
  return (
    product[field] === categoryId || meta[field] === categoryId || brandMeta[field] === categoryId
  );
}
