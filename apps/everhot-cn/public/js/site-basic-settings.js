...
(function () {
  ...
  var SETTINGS_URL=API_BASE+'/api/v2/sites/'+encodeURIComponent(SITE_CODE)+'/basic-settings';

  function dataOf(payload){return payload&&payload.data?payload.data:payload;}
  function section(settings,name){return settings&&settings[name]&&typeof settings[name]==='object'?settings[name]:{};}
  function pick(){
  ...
  function runtimeUrl(value,baseUrl){
    ...
    try{return new URL(src,pick(baseUrl,window.location.origin)).href;}catch(_){return src;}
  }
  ...
  function setHtml(selector,value){
    if(!value)return;
    document.querySelectorAll(selector).forEach(function(node){node.innerHTML=value;});
  }
  function setAttr(selector,attr,value){
    if(!value)return;
    document.querySelectorAll(selector).forEach(function(node){node.setAttribute(attr,value);});
  }
  function setMeta(selector,value){var node=document.querySelector(selector);if(node&&value)node.setAttribute('content',value);}
  function setLink(selector,attr,value){var node=document.querySelector(selector);if(node&&value)node.setAttribute(attr||'href',value);}
    ...
    document.querySelectorAll('script[type="application/ld+json"]').forEach(function(node){
      ...
      function patch(value){
        ...
        if(value['@type']==='Organization'){
          if(pick(seo.organizationName,identity.siteName))value.name=pick(seo.organizationName,identity.siteName);
          if(siteUrl)value.url=siteUrl;
          if(pick(seo.organizationLogo,identity.logoUrl))value.logo=runtimeUrl(pick(seo.organizationLogo,identity.logoUrl),siteUrl);
        ...
        if(value.isPartOf&&typeof value.isPartOf==='object'&&value.isPartOf['@type']==='WebSite'){
          if(siteName)value.isPartOf.name=siteName;
          if(siteUrl)value.isPartOf.url=siteUrl;
        }
        Object.keys(value).forEach(function(key){patch(value[key]);});
      }
      patch(parsed);
  ...
  function applyHead(settings){
    ...
    setMeta('meta[property="og:site_name"]',pick(seo.ogSiteName,identity.siteName,seo.organizationName));
    setMeta('meta[property="og:image"]',runtimeUrl(pick(seo.defaultOgImage),canonical));
    setMeta('meta[name="twitter:image"]',runtimeUrl(pick(seo.defaultTwitterImage,seo.defaultOgImage),canonical));
    setLink('link[rel="icon"][sizes="16x16"]','href',runtimeUrl(pick(identity.favicon16Url),canonical));
    setLink('link[rel="icon"][sizes="32x32"]','href',runtimeUrl(pick(identity.favicon32Url),canonical));
    setLink('link[rel="icon"][sizes="any"]','href',runtimeUrl(pick(identity.faviconIcoUrl),canonical));
    setLink('link[rel="apple-touch-icon"]','href',runtimeUrl(pick(identity.appleTouchIconUrl),canonical));
  ...
  function applyShell(settings){
    ...
    if(whiteLogoUrl){
      document.querySelectorAll('.footer-brand .logo').forEach(function(node){node.style.backgroundImage='url("'+whiteLogoUrl+'")';});
    }
    setText('.ev-util-locale',pick(identity.localeLabel));
    setAttr('.ev-group-link, .ev-drawer-group','href',pick(organization.groupSiteUrl,organization.operatorGroupUrl));
    setText('.ev-group-link',pick(organization.operatorGroupName)?pick(organization.operatorGroupName)+' ↗':'');
    ...
    var telHref=pick(contact.customerServiceTelHref,hotline?'tel:'+hotline.replace(/[^\d+]/g,''):'');
    if(hotline){
      document.querySelectorAll('a[href^="tel:"]').forEach(function(node){
        node.href=telHref;
        if(/400|致电|客服|热线|电话/i.test(node.textContent||''))node.textContent='致电 '+hotline;
      });
    }
  }
  ...
  function applyHome(settings){
    ...
    setHtml('.hero-content h1',[pick(claims.heroTitleLine1),pick(claims.heroTitleLine2)].filter(Boolean).map(esc).join('<br>'));
  ...
  function loadSettings(){
    ...
    fetch(SETTINGS_URL,{cache:'no-store',headers:{Accept:'application/json'}})
      ...
      .then(function(payload){applySettings(dataOf(payload));})
      .catch(function(){});
  }
