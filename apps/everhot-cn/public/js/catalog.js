...
(function () {
  ...
  function installCatalog() {
    ...
    window.EVERHOT_CATALOG = {
      by: function (cat, sys) {
        return window.EVERHOT_PRODUCTS.filter(function (p) {
          return p.cat === cat && p.sys === sys;
        });
      },
      one: function (slug) {
        return (
          window.EVERHOT_PRODUCTS.filter(function (p) {
            return p.slug === slug;
          })[0] || null
        );
      },
  ...
  function setRuntimeStatus(status) {
    ...
    try {
      window.dispatchEvent(
        new CustomEvent('everhot-products-status', { detail: { status: status } })
      );
    } catch (_) {}
  }
  ...
  function categoryText(product) {
    var meta =
      product && product.siteMeta && typeof product.siteMeta === 'object' ? product.siteMeta : {};
    var productBinding =
      meta.productCategoryBinding && typeof meta.productCategoryBinding === 'object'
        ? meta.productCategoryBinding
        : {};
    return [
  ...
  function categoryLeaf(product) {
    var parts = categoryPathValue(product)
      .split('/')
      .map(function (part) {
        return part.trim();
      })
  ...
  function productSiteCategory(product) {
    var meta =
      product && product.siteMeta && typeof product.siteMeta === 'object' ? product.siteMeta : {};
    return meta.siteProductCategory && typeof meta.siteProductCategory === 'object'
      ? meta.siteProductCategory
      : {};
  }
  ...
  function flattenSiteCategories() {
    var tree = Array.isArray(window.EVERHOT_SITE_PRODUCT_CATEGORY_TREE)
      ? window.EVERHOT_SITE_PRODUCT_CATEGORY_TREE
      : [];
    var items = Array.isArray(window.EVERHOT_SITE_PRODUCT_CATEGORIES)
    ...
    function walk(row, chain) {
      ...
      out.push({
        row: row,
        chain: next,
        tokens: next.map(categoryToken).filter(Boolean),
        path: normPath(
          next
            .map(function (item) {
              return item.name || item.websiteCategory || item.code || item.slug;
            })
            .filter(Boolean)
            .join(' / ')
        ),
      });
      (Array.isArray(row.children) ? row.children : []).forEach(function (child) {
        walk(child, next);
      });
    }
    if (tree.length) {
  ...
  function currentCatalogTokens() {
    ...
    if (idx < 0) return [];
    var tail = parts.slice(idx + 1).filter(function (part) {
      return part !== 'detail';
    });
    return tail;
  }
  function catalogCategoryContext() {
    ...
    var target = flat.filter(function (item) {
      return (
        ...
        item.tokens.every(function (token, index) {
    ...
    flat.forEach(function (item) {
      var sameBranch = target.tokens.every(function (token, index) {
        return item.tokens[index] === token;
      });
  ...
  function productMatchesCatalogCategory(product, context) {
    ...
    var paths = [product && product.websiteCategoryPath, meta.websiteCategoryPath, category.path]
      .map(normPath)
      .filter(Boolean);
  ...
  function isRuntimeProductForSite(product) {
    var brand = String((product && product.brand) || '')
      .trim()
      .toLowerCase();
  ...
  function loadRuntimeProducts() {
    ...
    if (!shouldUseRuntimeProducts()) {
      installCatalog();
      setRuntimeStatus('static');
      return Promise.resolve(false);
    }
    if (!window.fetch) {
      window.EVERHOT_PRODUCTS = [];
      installCatalog();
      setRuntimeStatus('api-error');
      return Promise.resolve(false);
    }
    ...
    window.EVERHOT_PRODUCTS_READY = Promise.all([categoriesReady, productsReady])
      .then(function (results) {
        ...
        if (Array.isArray(items)) {
          window.EVERHOT_PRODUCTS = items.map(normalizeRuntimeProduct).filter(function (p) {
            return p.slug && isRuntimeProductForSite(p);
          });
          installCatalog();
  ...
  function officialDetailBlock(product) {
    ...
    return (
      '<section class="section pd-official-detail-section"><div class="container">' +
      '<div class="pd-official-detail-body">' +
      html +
      '</div>' +
      '</div></section>'
    );
  ...
  function art(p) {
    ...
    if (icon)
      return (
        '<img class="pc-media-icon-img" src="' +
        e(icon) +
        '" alt="' +
        e(p.name || '产品图标') +
        '" loading="lazy" decoding="async">'
      );
    return typeof window.EVERHOT_ART === 'function'
      ? window.EVERHOT_ART(p)
      : '<span class="pc-media-emoji">' + e(p.icon || '🔧') + '</span>';
  }
  ...
  function prodIco(p) {
    ...
    if (icon)
      return (
        '<img class="pc-inline-icon-img" src="' +
        e(icon) +
        '" alt="" loading="lazy" decoding="async">'
      );
    var f = window.EVERHOT_ICON;
    if (!f) return e(p.icon || '🔧');
    var n = (p && (p.name || '')) + ' ' + (p && (p.series || ''));
    var name = /热泵|空气能/.test(n)
      ? 'fan'
      : /壁挂炉|燃气|冷凝|采暖热水/.test(n)
        ? 'flame'
        : /储热|水箱|容积|无槽/.test(n)
          ? 'drop'
          : p && p.sys === 'heating-cooling'
            ? 'thermo'
            : 'drop';
    return f(name);
  }
  ...
  function galleryBlock(p) {
    ...
    return (
      '<section class="section pd-gallery"><div class="container">' +
  ...
  function badges(p) {
    ...
    return (
      ...
      p.badges
        .slice(0, 1)
        .map(function (b) {
          return '<span class="pc-badge">' + e(b) + '</span>';
        })
        .join('') +
      '</div>'
    );
  ...
  function renderCatalogGrid(g, list) {
    ...
    try {
      var qsS = new URLSearchParams(location.search).get('series');
      if (qsS && seriesList.indexOf(qsS) >= 0) state.f = qsS;
    } catch (_) {}
    if (seriesList.length > 1 && g.parentNode) {
      bar = document.createElement('div');
      bar.className = 'cat-filter';
      bar.innerHTML =
        '<div class="cat-filter-chips"><button class="cat-chip is-on" data-f="__all">全部</button>' +
        seriesList
          .map(function (s) {
            return '<button class="cat-chip" data-f="' + e(s) + '">' + e(s) + '</button>';
          })
          .join('') +
      ...
      var sub =
        state.f === '__all'
          ...
          : list.filter(function (p) {
              return (p.series || '其他') === state.f;
            });
      g.innerHTML = sub.length ? runtimeNotice() + sub.map(card).join('') : emptyState();
      if (bar) {
        bar.querySelector('.cat-count').textContent = '共 ' + sub.length + ' 款';
        bar.querySelectorAll('.cat-chip').forEach(function (b) {
          b.classList.toggle('is-on', b.getAttribute('data-f') === state.f);
        });
      }
    }
    if (bar) {
  ...
  function emptyState() {
    return (
      ...
      '<div class="catalog-empty-act"><a class="btn btn-brand" href="' +
      BASE +
      '/find-a-pro/">查找经销商</a>' +
      '<a class="btn btn-outline" href="' +
      BASE +
      '/products/">浏览全部产品</a></div>' +
      '</div>'
    );
  }
  ...
  function loadingState() {
    return (
      '<div class="catalog-empty"><p class="catalog-empty-tt">正在读取最新产品目录</p>' +
  ...
  function runtimeNotice() {
    ...
    return (
      '<div class="catalog-empty"><p class="catalog-empty-tt">产品 API 暂时不可用</p>' +
  ...
  function renderGrids() {
    document.querySelectorAll('[data-catalog]').forEach(function (g) {
      ...
      if (categoryContext) {
        list = list.filter(function (p) {
          return productMatchesCatalogCategory(p, categoryContext);
        });
      } else if (key[2]) {
        list = list.filter(function (p) {
          return (p.series || '') === key[2];
        });
      }
      if (!list.length) {
        g.innerHTML = emptyState();
        return;
      }
    ...
    document.querySelectorAll('[data-featured]').forEach(function (g) {
      ...
      var list = window.EVERHOT_PRODUCTS.filter(function (p) {
        return p.cat === cat;
      });
  ...
  function highlightStats(p) {
    return (p.highlights || [])
      .map(function (h) {
        return (
          '<div class="pd-stat"><span class="pd-stat-val">' +
          e(displayMetricValue(h.value)) +
  ...
  function related(p) {
    ...
    if (!sib.length && window.EVERHOT_PRODUCTS_STATUS === 'runtime') {
      sib = window.EVERHOT_CATALOG.by(p.cat, p.sys)
        .filter(function (x) {
  ...
  function certBlock(p) {
    ...
    return (
      ...
      list
        .map(function (c) {
          return '<span class="cert-chip"><span class="cert-tick">✓</span>' + e(c) + '</span>';
        })
        .join('') +
      '</div>' +
      '</div></section>'
  ...
  function docBlock(p) {
    ...
    var resourcesUrl =
      BASE +
      '/professionals/' +
      (p.cat === 'commercial' ? 'commercial' : 'residential') +
      '/resources/';
    if (docs.length) {
      return (
        '<section class="section section-alt"><div class="container">' +
        ...
        docs
          .map(function (doc) {
            return (
              ...
              e(doc.url) +
              '" target="_blank" rel="noopener noreferrer">下载 ' +
              e(doc.filename) +
              '</a>'
            );
          })
  ...
  function faqList(p) {
    ...
    var f0 = (p.features || [])[0];
    if (f0) {
      arr.push({ q: '什么是' + f0.title + '？', a: f0.desc });
    }
    arr.push({
      q: '如何购买与安装 ' + p.name + '？',
      a: '恒热授权经销商提供免费上门勘测、选型设计与安装服务，点击「查找授权经销商」即可预约。',
    });
    arr.push({
      q: p.name + ' 提供怎样的质保？',
      a: '恒热产品享受厂家质保，具体年限以产品保修政策与设备铭牌为准，详见保修政策页。',
    });
    return arr;
  }
  function faqBlock(p) {
    ...
    return (
      ...
      '<div class="section-head"><div class="eyebrow">常见问题</div><h2>关于 ' +
      e(p.name) +
      '</h2></div>' +
      '<div class="faq-list">' +
      list
  ...
  function positioningBlock(p) {
    var pos = p && p.positioning;
    if (!pos) return '';
    var vp = pos.valueProposition || '',
      pains = pos.painPoints || [],
      scn = pos.scenarios || [];
    if (!vp && !pains.length && !scn.length) return '';
    var html =
      '<section class="section"><div class="container">' +
      '<div class="section-head"><div class="eyebrow">为谁而生</div><h2>' +
      e(p.name) +
      ' 的定位</h2></div>';
    if (vp) {
      html += '<p class="pd-value-prop">' + e(vp) + '</p>';
    }
    if (pains.length) {
      html +=
        '<h4 class="pd-pos-h">解决的痛点</h4><ul class="pd-pos-list">' +
        pains
          .map(function (x) {
            return '<li>' + e(x) + '</li>';
          })
    ...
    if (scn.length) {
      html +=
        '<h4 class="pd-pos-h">适用场景</h4><ul class="pd-pos-list">' +
        scn
          .map(function (x) {
            return '<li>' + e(x) + '</li>';
          })
  ...
  function renderDetail() {
    ...
    if (!p) {
      host.innerHTML =
        '<section class="page-hero"><div class="container"><div class="eyebrow">产品中心</div>' +
        '<h1>未找到该产品</h1><p>请返回产品中心选择具体型号。</p></div></section>' +
        '<section class="section"><div class="container"><a class="btn btn-brand btn-lg" href="' +
        BASE +
        '/products/">返回产品中心</a></div></section>';
      return;
    }
    var cn = catName(p);
    ...
    var html =
      ...
      (imgSrc(p)
        ...
        : '<div class="pd-hero-art">' + art(p) + '</div>') +
      '</div>' +
      '<div class="pd-hero-text">' +
      '<div class="pd-crumb"><a href="' +
      BASE +
      '/">首页</a> / <a href="' +
      BASE +
      '/products/">产品中心</a> / <a href="' +
      cn.sysPath +
      '">' +
      e(cn.c + cn.s) +
      '</a></div>' +
      ...
      '</h1>' +
      '<p class="pd-series">' +
      e(productHeroSeriesText(p)) +
      '</p>' +
      '<p class="pd-tagline">' +
      e(p.tagline) +
      '</p>' +
      '<div class="pd-hero-actions"><a class="btn btn-light btn-lg" href="' +
      BASE +
      '/find-a-pro/">预约经销商选型</a>' +
    ...
    html += galleryBlock(p);
    if (window.EVERHOT_SHOW_PRODUCT_POSITIONING) {
      html += positioningBlock(p);
    }
    if ((p.highlights || []).length) {
      html +=
        '<section class="pd-stats-band"><div class="container pd-stats">' +
        highlightStats(p) +
        '</div></section>';
    }
    if ((p.features || []).length) {
      html +=
        '<section class="section"><div class="container">' +
        '<div class="section-head"><div class="eyebrow">产品亮点</div><h2>为什么选择 ' +
        e(p.name) +
        '</h2></div>' +
        '<div class="pf-grid">' +
        featureCards(p) +
    ...
    if ((p.specs || []).length || sImg) {
      html +=
        '<section class="section section-alt" id="ev-specsheet"><div class="container">' +
        '<div class="section-head"><div class="eyebrow">技术规格</div><h2>规格参数</h2><p>具体型号与参数以经销商报价及产品铭牌为准（数据后台可更新）。</p></div>';
      if ((p.specs || []).length) {
        html += '<table class="spec-table"><tbody>' + specRows(p) + '</tbody></table>';
      }
      if (sImg) {
        html +=
          '<figure class="pd-specimg"><img src="' +
          e(sImg) +
          '" alt="' +
          e(p.name) +
          ' 产品参数图" loading="lazy" decoding="async"><figcaption>厂商产品参数图，仅供参考</figcaption></figure>';
      }
      html += '</div></section>';
    }
    html += certBlock(p);
    ...
    html += faqBlock(p);
    html +=
      '<section class="section-cta"><div class="container"><h2>需要为您的项目选型？</h2>' +
      '<p>恒热授权经销商提供免费勘测、方案设计与报价。</p>' +
      '<a class="btn btn-light btn-lg" href="' +
      BASE +
      '/find-a-pro/">查找授权经销商</a></div></section>';
    html += related(p);
    ...
    try {
      var ld = {
        ...
        additionalProperty: (p.specs || []).map(function (s) {
          return { '@type': 'PropertyValue', name: s.k, value: s.v };
        }),
      };
      var sc = document.createElement('script');
      sc.type = 'application/ld+json';
  ...
  function renderCompare() {
    ...
    function pool() {
      return window.EVERHOT_CATALOG.by(state.cat, state.sys);
    }
    ...
    function specVal(p, k) {
      var f = (p.specs || []).filter(function (s) {
        return s.k === k;
      })[0];
      return f ? f.v : '—';
    }
    function rowDiffers(picks, k) {
      ...
      for (var i = 1; i < picks.length; i++) {
        if (specVal(picks[i], k) !== first) return true;
      }
      return false;
    }
    ...
    function readUrl() {
      try {
        ...
        var valid = pk
          .filter(function (sl) {
            return !!window.EVERHOT_CATALOG.one(sl);
          })
          .slice(0, 4);
    ...
    function draw() {
      ...
      h +=
        '<div class="cmp-seg" data-seg="cat">' +
        Object.keys(CAT)
          .map(function (c) {
            return (
              '<button class="cmp-seg-btn' +
              (state.cat === c ? ' is-on' : '') +
              '" data-cat="' +
              c +
              '">' +
        ...
        '</div>';
      h +=
        '<div class="cmp-seg" data-seg="sys">' +
        Object.keys(SYS)
          .map(function (sy) {
            return (
              '<button class="cmp-seg-btn' +
      ...
      h +=
        ...
        (state.diffOnly ? ' is-on' : '') +
        '"><input type="checkbox" data-diff' +
        (state.diffOnly ? ' checked' : '') +
        '><span>只看差异</span></label>';
        ...
        list
          .map(function (p) {
            ...
            return (
              ...
              '"' +
              (on ? ' checked' : '') +
              '><span class="cmp-pick-ic">' +
              prodIco(p) +
              '</span><span class="cmp-pick-name">' +
              e(p.name) +
              '</span></label>'
            );
          })
      ...
      if (picks.length) {
        ...
        var shown = keys.filter(function (k) {
          return !(state.diffOnly && canDiff) || rowDiffers(picks, k);
        });
        h +=
          '<div class="cmp-table-wrap"><table class="cmp-table"><thead><tr><th>规格</th>' +
          picks
            .map(function (p) {
              return (
                '<th><a href="' +
        ...
        shown.forEach(function (k) {
          ...
          h +=
            '<tr' +
            (diff ? ' class="cmp-row-diff"' : '') +
            '><th>' +
            e(k) +
            (diff ? '<span class="cmp-diff-dot" title="各产品存在差异"></span>' : '') +
            '</th>' +
            picks
              .map(function (p) {
                return '<td>' + e(specVal(p, k)) + '</td>';
              })
              .join('') +
            '</tr>';
        });
        if (state.diffOnly && canDiff && !shown.length) {
          h +=
            '<tr><td colspan="' +
            (picks.length + 1) +
            '" class="cmp-empty" style="padding:18px">所选产品在已收录规格上完全一致。</td></tr>';
        }
        h +=
          '<tr><th></th>' +
          picks
            .map(function (p) {
              return (
                '<td><a class="btn btn-brand" style="font-size:12px;padding:7px 14px" href="' +
                productUrl(p) +
                '">查看详情</a></td>'
              );
            })
      ...
      host.querySelectorAll('[data-sys]').forEach(function (b) {
        b.addEventListener('click', function () {
      ...
      if (diffBox)
        diffBox.addEventListener('change', function () {
          state.diffOnly = diffBox.checked;
          draw();
        });
      ...
      host.querySelectorAll('input[data-slug]').forEach(function (cb) {
        cb.addEventListener('change', function () {
          ...
          if (cb.checked) {
            ...
            if (state.picked.indexOf(sl) < 0) state.picked.push(sl);
          } else {
            state.picked = state.picked.filter(function (x) {
              return x !== sl;
            });
          }
          draw();
  ...
  function renderLoading() {
    document.querySelectorAll('[data-catalog],[data-featured]').forEach(function (g) {
      g.innerHTML = loadingState();
    });
  ...
  function start() {
    installCatalog();
    renderLoading();
    loadRuntimeProducts().then(function () {
      boot();
    });
  }
