...
const context = {
  ...
  fetch: async (url) => {
    ...
    if (url === '/api/v2/sites/everhot/product-categories') {
      return { ok: true, json: async () => ({ success: true, data: { items: [], tree: categoryTree, total: 3 } }) };
    }
    assert.equal(url, '/api/v2/sites/everhot/products?locale=zh-CN');
    return { ok: true, json: async () => ({ success: true, data: { items: products, total: products.length } }) };
  },
  setTimeout,
  clearTimeout,
  Promise,
...
assert.deepEqual(calls, ['/api/v2/sites/everhot/product-categories', '/api/v2/sites/everhot/products?locale=zh-CN']);
