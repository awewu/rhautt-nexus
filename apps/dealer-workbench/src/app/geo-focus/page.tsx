  ...
  return (
    <div className="page-container">
        ...
        >
          ...
          >
            {[
              ...
              <div key={label} className="inset" style={{ padding: 12, textAlign: 'center' }}>
                <div
                  className="t-num"
                  style={{ fontSize: 22, color: 'var(--brand)', fontWeight: 800 }}
                >
                  {value}
                </div>
                <div className="t-xs" style={{ color: 'var(--t-tertiary)', marginTop: 3 }}>
                  {label}
                </div>
              </div>
            ))}
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            {PROBE_TYPES.map((p) => (
              <button
                key={p.key || 'all'}
                className={probeType === p.key ? 'btn btn-brand btn-sm' : 'btn btn-outline btn-sm'}
                onClick={() => setProbeType(p.key)}
              >
                {p.label} {p.key ? summary.byType?.[p.key] || 0 : (summary.total ?? tRows.length)}
              </button>
            ))}
          </div>
        </AsyncBoundary>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 20, flexWrap: 'wrap' }}>
        {['探测池负责找战场', '千问负责扩战场', '认知资产负责占战场', '引爆负责打穿战场'].map(
          (label) => (
            <span
              key={label}
              className="t-xs"
      ...
      <div className="card" style={{ padding: 20, marginBottom: 20 }}>
        ...
        >
          ...
          <div className="t-xs" style={{ color: 'var(--t-tertiary)' }}>
            答案占位率{' '}
            <span style={{ color: 'var(--brand)', fontWeight: 700 }}>{pct(brandPresenceRate)}</span>
          </div>
        </div>
        <AsyncBoundary
              ...
              return (
                <div key={k} style={{ flex: 1, textAlign: 'center' }}>
                  <div className="inset" style={{ padding: '14px 4px' }}>
                    <div
                      className="t-num"
                      style={{ fontSize: 24, fontWeight: 700, color: 'var(--brand)' }}
                    >
                      {v}
                    </div>
                    <div className="t-xs" style={{ color: 'var(--t-secondary)', fontWeight: 600 }}>
                      {label}
                    </div>
                    <div className="t-xs" style={{ color: 'var(--t-tertiary)', marginTop: 3 }}>
                      {desc}
      ...
      <div className="card" style={{ padding: 20, marginBottom: 20 }}>
        ...
        >
          ...
          <div className="t-xs" style={{ color: 'var(--t-tertiary)' }}>
            {selectedCategory} · 高价值点 {highPriorityTargets} 个
          </div>
        </div>
        <div style={{ display: 'flex', gap: 8, marginBottom: 16, flexWrap: 'wrap' }}>
          ...
          <input
            ...
            style={{ width: 90 }}
          />
          <button className="btn btn-brand" onClick={addTarget}>
            录入
          </button>
        </div>
        <AsyncBoundary
        ...
        >
          <div style={{ display: 'grid', gap: 8 }}>
            {tRows.map((t) => (
              ...
              >
                <div>
                  <div className="t-sm" style={{ fontWeight: 600, color: 'var(--t-strong)' }}>
                    {t.query}
                  </div>
                  <div className="t-xs" style={{ color: 'var(--t-tertiary)', marginTop: 3 }}>
                    {t.segment || '通用人群'} / {t.engine || '全部引擎'}
                    {t.region ? ` / ${t.region}` : ''}
                  </div>
                </div>
                <div className="t-xs" style={{ color: 'var(--t-secondary)' }}>
                  探测类型
                  <br />
                  <span style={{ color: 'var(--t-strong)', fontWeight: 600 }}>
                    {PROBE_TYPE_LABEL[t.probeType] || t.probeType || '品类问题'}
                  </span>
                </div>
                <div className="t-xs" style={{ color: 'var(--t-secondary)' }}>
                  意图阶段
                ...
                <div style={{ textAlign: 'right' }}>
                  <div className="t-xs" style={{ color: 'var(--brand)', fontWeight: 700 }}>
                    优先级 {Number(t.priorityScore).toFixed(0)}
                  </div>
                  <button
      ...
      >
        ...
        <div className="card" style={{ padding: 20 }}>
          ...
          <div className="inset" style={{ padding: 16, textAlign: 'center', marginBottom: 12 }}>
            <div className="t-num" style={{ fontSize: 34, color: 'var(--brand)', fontWeight: 800 }}>
              {igniteReadiness}
            </div>
            <div className="t-xs" style={{ color: 'var(--t-tertiary)' }}>
          ...
          <div style={{ display: 'grid', gap: 8 }}>
            {[
              ...
              >
                ...
                <span className="t-xs" style={{ color: 'var(--t-strong)', fontWeight: 700 }}>
                  {value}
                </span>
              </div>
            ))}
      ...
      {plays.length > 0 && (
        <div className="card" style={{ padding: 20 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 16 }}>
            ...
            <span className="t-lg" style={{ fontWeight: 600 }}>
              引爆 · 千问千面草稿（{plays.length}）
            </span>
          </div>
          <div style={{ display: 'grid', gap: 12 }}>
            {plays.map((p) => (
              <div key={p.targetId} className="inset">
                <div className="t-sm" style={{ color: 'var(--t-strong)', fontWeight: 600 }}>
                  {p.query}{' '}
                  <span className="t-xs" style={{ color: 'var(--t-tertiary)', fontWeight: 400 }}>
