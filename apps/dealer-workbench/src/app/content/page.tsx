...
function statusPill(status: string) {
  ...
  return (
    <span className={`status-pill status-pill-${tone}`}>{STATUS_LABEL[status] || status}</span>
  );
}
...
function targetSummary(row: ContentRow) {
  if (
    row.source?.type === 'geo_experiment' ||
    row.sourceType === 'geo_experiment' ||
    row.sourceType === 'geo_gap'
  )
    return '提升 AI 可见度与高意向线索';
...
export default function ContentPage() {
  ...
  const factSources = useSWR(factTarget ? ['content:fact-sources', factQuery] : null, () =>
  ...
  function toggleFactSource(source: FactSource) {
    ...
    setSelectedRefs((current) => {
      ...
      if (current.some((ref) => refKey(ref) === key))
        return current.filter((ref) => refKey(ref) !== key);
  ...
  async function uploadEvidenceFile(file: File | null) {
    ...
    try {
      ...
      setSelectedRefs((current) =>
        current.some((ref) => refKey(ref) === refKey(nextRef)) ? current : [...current, nextRef]
      );
  ...
  async function saveFactRefs() {
    ...
    try {
      ...
      toast(
        result?.gate?.passed ? '事实源已绑定并通过校验' : '事实源已绑定，仍有待校验项',
        result?.gate?.passed ? 'success' : 'info'
      );
      closeFactBinder();
      list.mutate();
    } catch (e) {
  ...
  const publishTasks: PublishTask[] = Array.isArray(taskList.data?.tasks)
    ? taskList.data.tasks
    : [];
  ...
  const stats = useMemo(() => {
    ...
    const blocked = rows.filter(
      (item) => item.status !== 'published' && !factGatePassed(item)
    ).length;
    const publishTasksOpen = publishTasks.filter(
      (task) => task.status !== 'published' && task.status !== 'cancelled'
    ).length;
    const retro = rows.filter(
      (item) => item.status === 'published' && !item.retrospective?.done
    ).length;
    const readyToPublish = rows.filter(
      (item) => item.status === 'approved' && factGatePassed(item) && !item.openPublishTask
    ).length;
    return {
      draft,
      review,
  ...
  const sourceItems: FactSource[] = Array.isArray(factSources.data?.items)
    ? factSources.data.items
    : [];
    ...
    .filter(
      (item) =>
        ...
        (!factGatePassed(item) ||
          ...
          (item.status === 'approved' && !item.openPublishTask))
    )
    .slice(0, 4);
      ...
      <section className="card-elevated content-factory-workbench">
        <div className="content-factory-hero">
          ...
          <div className="content-factory-metrics" aria-label="内容工厂关键指标">
            <span>
              <strong>{rows.length}</strong> 在库内容
            </span>
            <span>
              <strong>{stats.blocked}</strong> 事实源阻塞
            </span>
            <span>
              <strong>{stats.publishTasksOpen}</strong> 发布执行中
            </span>
          </div>
        </div>
        ...
        <div className="content-factory-pipeline" aria-label="内容生产流水线">
          {pipelineStages.map((stage) => {
            ...
            return (
              ...
              >
                <span className="content-factory-stage__icon">
                  <Icon size={16} />
                </span>
        ...
        <div className="content-factory-layout">
                    ...
                    <article key={c.id} className="content-factory-item">
                      <div className="content-factory-item__body">
                        <div className="content-factory-item__title">
                          ...
                          <span
                            className={`content-factory-next content-factory-next--${c.nextAction?.tone || 'neutral'}`}
                          >
                            <Flag size={12} />
                            {nextActionLabel(c)}
                          </span>
                        ...
                        <div className="content-factory-item__meta">
                          <span>
                            {KIND_OPTIONS.find(([value]) => value === c.kind)?.[1] || c.kind}
                          </span>
                          <span>{c.channel}</span>
                          <span className={gatePassed ? 'is-ok' : 'is-blocked'}>
                            {gatePassed
                              ? `${c.factGate?.verifiedCount ?? verifiedFacts} 个已校验事实源`
                              : c.factGate?.reason || '缺事实源/未校验'}
                          </span>
                          {latestTask ? (
                            <span
                              className={latestTask.status === 'published' ? 'is-ok' : 'is-blocked'}
                            >
                              {taskLabel(latestTask)}
                            </span>
                          ) : null}
                          <span>
                            <UserRound size={12} />
                            {currentOwner(c, openTask)}
                          </span>
                          {c.aging?.daysInCurrentStatus != null ? (
                            <span className={c.aging.overdue ? 'is-blocked' : ''}>
                              <Clock3 size={12} />
                              停留 {c.aging.daysInCurrentStatus} 天
                            </span>
                          ) : null}
                          {c.retrospective?.needsGeoRetest ? (
                            <span className="is-blocked">待 GEO 复测</span>
                          ) : null}
                        </div>
                      ...
                      <div className="content-factory-item__actions">
                        ...
                        <button
                          className={
                            c.nextAction?.tone === 'danger'
                              ? 'btn btn-warning btn-sm'
                              : c.nextAction?.tone === 'success'
                        ...
                        <button
                          className={
                            gatePassed ? 'btn btn-outline btn-sm' : 'btn btn-warning btn-sm'
                          }
                          onClick={() => openFactBinder(c)}
                        >
                          <Link2 size={13} />
                          事实源
                        </button>
                        {c.status === 'draft' || c.status === 'rejected' ? (
                          <button
                            className="btn btn-outline btn-sm"
                            onClick={() => act(() => content.submit(c.id), '已送审')}
                            disabled={!gatePassed}
                          >
                            提交审核
                          </button>
          ...
          <aside className="content-factory-aside">
                ...
                {urgentRows.length ? (
                  urgentRows.map((item) => (
                    <button
                      key={item.id}
                      type="button"
                      onClick={() => handleNextAction(item, item.openPublishTask || null)}
                    >
                      <span>
                        <strong>{item.title}</strong>
                        <small>
                          {contentSource(item)} {'\u00b7'}{' '}
                          {item.factGate?.reason || nextActionLabel(item)}
                        </small>
                      </span>
                  ...
                  <div className="content-factory-fact-empty">
                    {
                      '\u6682\u65e0\u660e\u663e\u963b\u585e\u3002\u53ef\u4ee5\u65b0\u5efa brief\uff0c\u6216\u4ece\u6587\u6848 Copilot \u9001\u5165\u8349\u7a3f\u3002'
                    }
                  </div>
                )}
              </div>
            ...
            <div className="content-factory-create content-factory-quickstart">
              ...
              <p>
                {
                  '\u7528\u4e8e\u8865\u5145\u4eba\u5de5\u9009\u9898\u3001\u6d3b\u52a8\u8bf4\u660e\u6216\u6e20\u9053 brief\u3002\u521b\u5efa\u540e\u8fdb\u5165\u4e3b\u5217\u8868\uff0c\u518d\u7ed1\u5b9a\u4e8b\u5b9e\u6e90\u4e0e\u5ba1\u6838\u3002'
                }
              </p>
            ...
            <div className="content-factory-gate">
              ...
              <p>
                {
                  '\u5185\u5bb9\u5fc5\u987b\u5148\u9009\u62e9\u4ea7\u54c1\u4e8b\u5b9e\u3001\u5356\u70b9\u8bc1\u636e\u6216\u8d44\u6599\u51ed\u8bc1\uff0c\u5e76\u7531\u540e\u7aef\u6821\u9a8c\u5199\u5165\u540e\uff0c\u624d\u80fd\u521b\u5efa\u53d1\u5e03\u4efb\u52a1\uff1b\u53d1\u5e03\u5b8c\u6210\u5e76\u56de\u586b\u51ed\u8bc1\u540e\u624d\u7b97\u5df2\u53d1\u5e03\u3002'
                }
              </p>
            </div>
          </aside>
        ...
        {briefOpen && (
          <div className="content-factory-modal" onClick={closeBrief}>
            <div
              className="content-factory-dialog content-factory-dialog--md"
              role="dialog"
              aria-modal="true"
            ...
            >
              <div className="content-factory-dialog__head">
                <div>
                  ...
                  <h3 id="content-brief-title">{'\u8fdb\u5165\u5185\u5bb9\u6d41\u6c34\u7ebf'}</h3>
                </div>
                <button
                  className="btn btn-ghost btn-sm icon-only"
                  onClick={closeBrief}
                  aria-label={'\u5173\u95ed\u65b0\u5efa Brief'}
                >
                  <X size={16} />
                </button>
              </div>
                ...
                <div className="content-factory-formgrid content-factory-formgrid--modal">
                  ...
                  <label className="content-factory-field">
                    ...
                    <input
                      ...
                      placeholder="geo / wechat / website"
                    />
                  </label>
                </div>
                <label className="content-factory-field">
                  <span>Brief / 正文</span>
                  <textarea
                    className="textarea content-factory-dialog__textarea"
                    value={f.body}
                    onChange={(e) => setF({ ...f, body: e.target.value })}
                    placeholder="写清楚选题背景、目标人群、要回答的问题和必须使用的事实。"
                  />
                </label>
                <label className="content-factory-field">
                  <span>事实源 ID（可选）</span>
                  <input
                    className="input"
                    value={f.factRefs}
                    onChange={(e) => setF({ ...f, factRefs: e.target.value })}
              ...
              <div className="content-factory-dialog__foot">
                ...
                <div>
                  ...
                  <button className="btn btn-brand btn-sm" onClick={create}>
                    <PenTool size={14} />
                    {'\u65b0\u5efa\u8349\u7a3f'}
                  </button>
                </div>
        ...
        {editTarget && (
          <div className="content-factory-modal" onClick={closeEditor}>
            <div
              className="content-factory-dialog content-factory-dialog--lg"
              role="dialog"
              aria-modal="true"
            ...
            >
              <div className="content-factory-dialog__head">
                <div>
                  ...
                  <h3 id="content-edit-title">{editTarget.title}</h3>
                </div>
                <button
                  className="btn btn-ghost btn-sm icon-only"
                  onClick={closeEditor}
              ...
              <div className="content-factory-dialog__body content-factory-dialog__body--editor">
                ...
                <label className="content-factory-field">
                  <span>正文 / Brief</span>
                  <textarea
                    className="textarea content-factory-dialog__textarea content-factory-dialog__textarea--lg"
                    value={editForm.body}
                    onChange={(e) => setEditForm({ ...editForm, body: e.target.value })}
                    placeholder="正文 / brief"
                  />
                </label>
              </div>
              <div className="content-factory-dialog__foot">
        ...
        {publishTarget && (
          <div className="content-factory-modal" onClick={closePublishTask}>
            <div
              ...
              role="dialog"
              aria-modal="true"
              aria-labelledby="content-publish-title"
              onClick={(event) => event.stopPropagation()}
            >
              <div className="content-factory-dialog__head">
                <div>
                  ...
                  <h3 id="content-publish-title">{publishTarget.title}</h3>
                </div>
                <button
                  className="btn btn-ghost btn-sm icon-only"
                  onClick={closePublishTask}
                  aria-label="关闭发布任务"
                >
                  <X size={16} />
                </button>
              ...
              <div className="content-factory-dialog__body">
                <label className="content-factory-field">
                  ...
                  >
                    {PUBLISH_CHANNEL_OPTIONS.map(([value, label]) => (
                      <option key={value} value={value}>
                        {label}
                      </option>
                    ))}
                ...
                <label className="content-factory-field">
                  ...
                  <input
                    ...
                    placeholder="例如：Rheem 官网新闻、官方公众号"
                  />
                </label>
                <div className="content-factory-formgrid content-factory-formgrid--modal">
                  <label className="content-factory-field">
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setPublishForm({ ...publishForm, publishMode: e.target.value })
                      }
                    >
                ...
                <label className="content-factory-field">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setPublishForm({ ...publishForm, scheduledAt: e.target.value })
                    }
                  />
                </label>
              </div>
              <div className="content-factory-dialog__foot">
                <span>
                  {
                    '\u5ba1\u6838\u901a\u8fc7\u4e0d\u7b49\u4e8e\u5df2\u53d1\u5e03\uff1b\u6267\u884c\u6e20\u9053\u53d1\u5e03\u540e\u9700\u56de\u586b\u51ed\u8bc1\u3002'
                  }
                </span>
                <div>
                  <button className="btn btn-ghost btn-sm" onClick={closePublishTask}>
                    {'\u53d6\u6d88'}
                  </button>
                  <button
                    className="btn btn-brand btn-sm"
        ...
        {evidenceTarget && (
          <div className="content-factory-modal" onClick={() => setEvidenceTarget(null)}>
            <div
              ...
              role="dialog"
              aria-modal="true"
              aria-labelledby="content-evidence-title"
              onClick={(event) => event.stopPropagation()}
            >
              <div className="content-factory-dialog__head">
                <div>
                  ...
                  <h3 id="content-evidence-title">
                    {evidenceTarget.targetName || evidenceTarget.channel}
                  </h3>
                </div>
                <button
                  className="btn btn-ghost btn-sm icon-only"
              ...
              <div className="content-factory-dialog__body">
                <label className="content-factory-field">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setEvidenceForm({ ...evidenceForm, evidenceUrl: e.target.value })
                    }
                    placeholder="https://..."
                  />
                </label>
                <label className="content-factory-field">
                  ...
                  <textarea
                    ...
                    onChange={(e) =>
                      setEvidenceForm({ ...evidenceForm, evidenceNote: e.target.value })
                    }
                    placeholder="例如：已发布到公众号草稿箱并推送，或已同步到官网新闻栏目。"
                  />
                </label>
              </div>
              <div className="content-factory-dialog__foot">
                ...
                <div>
                  <button className="btn btn-ghost btn-sm" onClick={() => setEvidenceTarget(null)}>
                    {'\u53d6\u6d88'}
                  </button>
                  <button
                    className="btn btn-brand btn-sm"
                    onClick={completePublishTask}
                    disabled={
                      publishSaving ||
                      (!evidenceForm.evidenceUrl.trim() && !evidenceForm.evidenceNote.trim())
                    }
                  >
                    {publishSaving ? (
                      <Loader2 size={14} className="animate-spin" />
        ...
        {factTarget && (
          <div className="content-factory-modal" onClick={closeFactBinder}>
            <div
              ...
              role="dialog"
              aria-modal="true"
              aria-labelledby="content-fact-title"
              onClick={(event) => event.stopPropagation()}
            >
              <div className="content-factory-dialog__head">
                <div>
                  ...
                  <h3 id="content-fact-title">{factTarget.title}</h3>
                </div>
                <button
                  className="btn btn-ghost btn-sm icon-only"
                  onClick={closeFactBinder}
                  aria-label="关闭事实源绑定"
                >
                  <X size={16} />
                </button>
              ...
              <div className="content-factory-dialog__body content-factory-dialog__body--facts">
                <label className="content-factory-field">
                  ...
                  <input
                    ...
                    placeholder="搜索产品、SKU、卖点或证据"
                  />
                </label>
                <div
                  className="content-factory-fact-list content-factory-fact-list--modal"
                  aria-label="可选事实源"
                >
                  {factSources.isLoading ? (
                    ...
                    sourceItems.map((source) => {
                      ...
                      return (
                        <button
                          ...
                          title={
                            source.verified ? undefined : '可先绑定，但发布前仍需通过后端事实校验'
                          }
                        >
                          <span>
                    ...
                    <div className="content-factory-fact-empty">
                      暂无匹配事实源。可以换关键词搜索，或直接上传一份资料作为凭证。
                    </div>
                  )}
                </div>
                <div className="content-factory-upload-ref">
                  <label
                    className="btn btn-outline btn-sm"
                    htmlFor="content-factory-evidence-upload"
                  >
                    {evidenceUploading ? (
                  ...
                  />
                  <span>
                    支持产品手册、认证文件、参数表、活动说明等资料。上传后系统自动保存为事实源。
                  </span>
                </div>
                <div className="content-factory-selected-refs">
                  {selectedRefs.map((ref) => (
                    <button
                      ...
                      onClick={() =>
                        setSelectedRefs((current) =>
                          current.filter((item) => refKey(item) !== refKey(ref))
                        )
                      }
                    >
              ...
              <div className="content-factory-dialog__foot">
                ...
                <div>
                  <button className="btn btn-ghost btn-sm" onClick={closeFactBinder}>
                    {'\u53d6\u6d88'}
                  </button>
                  <button
                    className="btn btn-brand btn-sm"
                    onClick={saveFactRefs}
                    disabled={factSaving || !selectedRefs.length}
                  >
                    {factSaving ? (
                      <Loader2 size={14} className="animate-spin" />
                    ) : (
