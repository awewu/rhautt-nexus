...
export function navItemForPath(path: string | null): WorkbenchNavItem {
  ...
  if (path?.startsWith('/cockpit') || path?.startsWith('/cmo') || path?.startsWith('/strategy'))
    return byKey('cockpit');
  // 增长功能一级菜单
  if (
    path?.startsWith('/agentic-geo') ||
    path?.startsWith('/geo-focus') ||
    path?.startsWith('/growth/geo')
  )
    return byKey('geo');
  if (path?.startsWith('/insight')) return byKey('insight');
  if (
    path?.startsWith('/content') ||
    path?.startsWith('/growth/copywriter') ||
    path?.startsWith('/growth/content-assets') ||
    path?.startsWith('/growth/prompts') ||
    path?.startsWith('/growth/wechat')
  )
    return byKey('content');
